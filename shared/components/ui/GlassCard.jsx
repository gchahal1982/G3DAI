...
const StyledGlassCard = styled.div `
  ...
  `}
`;
export const GlassCard = ({ children, variant = 'surface', size = 'md', blur = 'medium', hover = false, interactive = false, loading = false, className, onClick, theme, style, ...props }) => {
    const mergedTheme = { ...baseGlassmorphismTheme, ...theme };
    return (<StyledGlassCard variant={variant} size={size} blur={blur} hover={hover} interactive={interactive} loading={loading} className={className} onClick={onClick} theme={mergedTheme} style={style} {...props}>
            {children}
        </StyledGlassCard>);
};
const StyledGlassButton = styled.button `
  ...
  `}
`;
export const GlassButton = ({ children, variant = 'primary', size = 'md', disabled = false, loading = false, fullWidth = false, icon, iconPosition = 'left', onClick, className, theme, style, type = 'button', ...props }) => {
    const mergedTheme = { ...baseGlassmorphismTheme, ...theme };
    return (<StyledGlassButton variant={variant} size={size} disabled={disabled || loading} loading={loading} fullWidth={fullWidth} onClick={onClick} className={className} theme={mergedTheme} style={style} type={type} {...props}>
...
const ErrorMessage = styled.span `
  ...
  font-size: 0.875rem;
`;
export const GlassInput = ({ type = 'text', placeholder, value, onChange, disabled = false, error = false, errorMessage, icon, iconPosition = 'left', fullWidth = false, className, theme, ...props }) => {
    const mergedTheme = { ...baseGlassmorphismTheme, ...theme };
    return (<InputWrapper fullWidth={fullWidth} className={className}>
            <StyledGlassInput type={type} placeholder={placeholder} value={value} onChange={(e) => onChange?.(e.target.value)} disabled={disabled} error={error} theme={mergedTheme} {...props}/>
            {error && errorMessage && (<ErrorMessage theme={mergedTheme}>{errorMessage}</ErrorMessage>)}
        </InputWrapper>);
};
...
const ModalTitle = styled.h2 `
  ...
  font-weight: 600;
`;
export const GlassModal = ({ isOpen, onClose, title, children, size = 'md', closeOnOverlayClick = true, className, theme, ...props }) => {
    const mergedTheme = { ...baseGlassmorphismTheme, ...theme };
    ...
    return (<ModalOverlay isOpen={isOpen} onClick={handleOverlayClick}>
            <ModalContent isOpen={isOpen} onClose={onClose} size={size} className={className} theme={mergedTheme} {...props}>
                {title && (<ModalHeader theme={mergedTheme}>
