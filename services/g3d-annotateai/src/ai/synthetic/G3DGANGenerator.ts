...
export class G3DGANGenerator {
    ...
    private async trainDiscriminator(
        ...
        try {
            ...
            const fakeBatch = await this.modelRunner.inference(model.generator, {
                input: noise,
                batchSize: config.batchSize
            });

            // Compute discriminator predictions
            const realScores = await this.modelRunner.inference(model.discriminator, {
                input: realBatch,
                batchSize: config.batchSize
            });

            const fakeScores = await this.modelRunner.inference(model.discriminator, {
                input: fakeBatch,
                batchSize: config.batchSize
            });
    ...
    public async generateImages(
        ...
        try {
            ...
            for (let batch = 0; batch < numBatches; batch++) {
                ...
                // Generate images
                const images = await this.modelRunner.inference(model.generator, {
                    input: noise,
                ...
                const scores = await this.modelRunner.inference(model.discriminator, {
                    input: images,
                    batchSize: currentBatchSize
                });
