...
export class G3DQualityMetrics {
    ...
    private async extractInceptionFeatures(data: any[]): Promise<Float32Array[]> {
        ...
        for (let i = 0; i < data.length; i += batchSize) {
            ...
            const batchFeatures = await this.modelRunner.inference(this.inceptionModel, {
                input: batch,
                layer: 'pool_3',
    ...
    private async getInceptionPredictions(data: any[]): Promise<Float32Array[]> {
        ...
        for (let i = 0; i < data.length; i += batchSize) {
            const batch = data.slice(i, i + batchSize);
            const batchPreds = await this.modelRunner.inference(this.inceptionModel, {
                input: batch,
    ...
    private async calculateLPIPS(data1: any[], data2: any[]): Promise<number[]> {
        ...
        for (let i = 0; i < Math.min(data1.length, data2.length); i += batchSize) {
            ...
            const batchScores = await this.modelRunner.inference(this.lpipsModel, {
                input1: batch1,
                input2: batch2,
