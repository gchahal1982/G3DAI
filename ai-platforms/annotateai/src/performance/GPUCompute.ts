 ...
 */
export class GPUCompute extends EventEmitter {
        ...
        for (const [binding, resource] of resources) {
            ...
            if (resource.type === 'buffer') {
                ...
                if (resource.data) {
                    ...
                    const sourceData = resource.data instanceof ArrayBuffer ? 
                        new Uint8Array(resource.data) : 
                        new Uint8Array(resource.data.buffer);
                    new Uint8Array(mappedRange).set(sourceData);
                    buffer.unmap();
                }
    ...
    getPeakMemoryUsage(): number {
        return this.metrics.peakMemoryUsage;
    }
