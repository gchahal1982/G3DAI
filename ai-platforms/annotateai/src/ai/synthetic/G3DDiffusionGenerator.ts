...
export class DiffusionGenerator {
    ...
    private createDepthPreprocessor(): any {
        return {
            process: async (image: ImageData, params: any = {}) => {
                ...
                const depthMap = await this.modelRunner.runInference('default', depthModel, {
                    input: image,
    ...
    private createPosePreprocessor(): any {
        return {
            process: async (image: ImageData, params: any = {}) => {
                ...
                const poseMap = await this.modelRunner.runInference('default', poseModel, {
                    input: image,
    ...
    private createSegmentationPreprocessor(): any {
        return {
            process: async (image: ImageData, params: any = {}) => {
                ...
                const segmentationMap = await this.modelRunner.runInference('default', segModel, {
                    input: image,
    ...
    private async runDiffusionProcess(
        ...
        try {
            ...
            for (let step = 0; step < steps; step++) {
                ...
                // Predict noise
                const noisePred = await this.modelRunner.runInference('default', model, {
                    latents,
                    timestep,
                    conditioning,
                    guidanceScale,
    ...
    private async decodeLatents(latents: Float32Array, config: DiffusionConfig): Promise<ImageData[]> {
        try {
            ...
            const decodedImages = await this.modelRunner.runInference('default', vaeDecoder, {
                latents,
                outputFormat: 'rgb',
