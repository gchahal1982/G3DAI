...
import { G3DSceneNode } from './G3DSceneManager';

// Light types
export enum G3DLightType {
...
export interface G3DLight {
    ...
    enabled: boolean;
    castShadow: boolean;
    shadowConfig?: G3DShadowConfig;
}

// Directional light
export interface G3DDirectionalLight extends G3DLight {
    type: G3DLightType.DIRECTIONAL;
    direction: vec3;
}

// Point light
export interface G3DPointLight extends G3DLight {
    type: G3DLightType.POINT;
    position: vec3;
    range: number;
    decay: number;
}
...
// Spot light
export interface G3DSpotLight extends G3DLight {
    type: G3DLightType.SPOT;
    position: vec3;
    direction: vec3;
...
// Area light
export interface G3DAreaLight extends G3DLight {
    type: G3DLightType.AREA;
    position: vec3;
    width: number;
    height: number;
    normal: vec3;
...
// Hemisphere light
export interface G3DHemisphereLight extends G3DLight {
    type: G3DLightType.HEMISPHERE;
    skyColor: vec3;
    groundColor: vec3;
    direction: vec3;
}
...
export interface G3DIBLLight extends G3DLight {
    type: G3DLightType.IBL;
    environmentMap: G3DCubeTexture;
    irradianceMap?: G3DCubeTexture;
    prefilterMap?: G3DCubeTexture;
    brdfLUT?: G3DTexture;
}

// Shadow configuration
export interface G3DShadowConfig {
    mapSize: number;
    bias: number;
    normalBias: number;
    radius: number;
...
export interface G3DCubeTexture {
    texture: GPUTexture | WebGLTexture;
    size: number;
}

// Light uniform buffer structure
export interface G3DLightUniforms {
...
// Lighting statistics
export interface G3DLightingStats {
    totalLights: number;
    activeLights: number;
...
export class G3DLightingSystem {
    private lights: Map<string, { light: G3DLight; node: G3DSceneNode }> = new Map();
    private shadowMaps: Map<string, G3DShadowMap> = new Map();
    private lightUniforms: G3DLightUniforms;
    private stats: G3DLightingStats = {
        totalLights: 0,
        activeLights: 0,
        shadowCasters: 0,
        shadowMapMemory: 0,
    ...
    addLight(id: string, light: G3DLight, node: G3DSceneNode): void {
    ...
    getLight(id: string): G3DLight | undefined {
        const lightData = this.lights.get(id);
    ...
    private createShadowMap(id: string, light: G3DLight): void {
    ...
    update(deltaTime: number): void {
        ...
        this.lights.forEach(({ light, node }, id) => {
            ...
            switch (light.type) {
                case G3DLightType.DIRECTIONAL:
                    if (directionalCount < this.config.maxDirectionalLights) {
                        this.updateDirectionalLight(light as G3DDirectionalLight, node, directionalCount++);
                    }
                    break;

                case G3DLightType.POINT:
                    if (pointCount < this.config.maxPointLights) {
                        this.updatePointLight(light as G3DPointLight, node, pointCount++);
                    }
                    break;

                case G3DLightType.SPOT:
                    if (spotCount < this.config.maxSpotLights) {
                        this.updateSpotLight(light as G3DSpotLight, node, spotCount++);
                    }
                    break;

                case G3DLightType.AREA:
                    if (areaCount < this.config.maxAreaLights) {
                        this.updateAreaLight(light as G3DAreaLight, node, areaCount++);
                    }
                    break;
            }
    ...
    private updateDirectionalLight(light: G3DDirectionalLight, node: G3DSceneNode, index: number): void {
    ...
    private updatePointLight(light: G3DPointLight, node: G3DSceneNode, index: number): void {
    ...
    private updateSpotLight(light: G3DSpotLight, node: G3DSceneNode, index: number): void {
    ...
    private updateAreaLight(light: G3DAreaLight, node: G3DSceneNode, index: number): void {
    ...
    private updateShadowMap(id: string, light: G3DLight, node: G3DSceneNode): void {
        const shadowMap = this.shadowMaps.get(id);
        ...
        switch (light.type) {
            case G3DLightType.DIRECTIONAL:
                this.updateDirectionalShadowMap(light as G3DDirectionalLight, node, shadowMap, config);
                break;

            case G3DLightType.SPOT:
                this.updateSpotShadowMap(light as G3DSpotLight, node, shadowMap, config);
                break;

            case G3DLightType.POINT:
                // Point lights would use cube shadow maps
                this.updatePointShadowMap(light as G3DPointLight, node, shadowMap, config);
                break;
        }
    ...
    private updateDirectionalShadowMap(
        light: G3DDirectionalLight,
        node: G3DSceneNode,
        shadowMap: G3DShadowMap,
        config: G3DShadowConfig
    ): void {
    ...
    getLightUniforms(): G3DLightUniforms {
        return this.lightUniforms;
    }

    getShadowMap(lightId: string): G3DShadowMap | undefined {
        return this.shadowMaps.get(lightId);
    }

    getStats(): G3DLightingStats {
        return { ...this.stats };
    }

    // Utility functions

    createDirectionalLight(color: vec3, intensity: number, direction: vec3): G3DDirectionalLight {
        return {
            type: G3DLightType.DIRECTIONAL,
    ...
    createPointLight(color: vec3, intensity: number, position: vec3, range: number): G3DPointLight {
        return {
            type: G3DLightType.POINT,
            color: vec3.clone(color),
            intensity,
...
export function createG3DLightingSystem(): G3DLightingSystem {
    return new G3DLightingSystem();
}