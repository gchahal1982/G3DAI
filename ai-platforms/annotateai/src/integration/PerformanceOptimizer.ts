...
export class PerformanceOptimizer {
    ...
    private memoryAllocations: Map<string, MemoryAllocation> = new Map();
    private drawBatches: Map<string, DrawBatch> = new Map();
    ...
    endFrame(): void {
        ...
        this.metrics.cpuTime = Date.now() - this.lastFrameTime;
    }
    ...
    createInstanceBatch(geometryId: string, materialId: string, capacity: number): DrawBatch {
    ...
    addInstance(batch: DrawBatch, transform: mat4, color?: Float32Array): boolean {
    ...
    updateInstanceBuffer(batch: DrawBatch, device: GPUDevice | WebGL2RenderingContext): void {
        ...
        if ('createBuffer' in device && 'queue' in device) {
            // WebGPU
...
export function createG3DPerformanceOptimizer(): PerformanceOptimizer {
