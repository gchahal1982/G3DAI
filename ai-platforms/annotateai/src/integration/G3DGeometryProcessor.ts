...
export class GeometryProcessor {
    ...
    createBox(width: number, height: number, depth: number, options?: GeometryOptions): Geometry {
        const hw = width / 2;
        const hh = height / 2;
        const hd = depth / 2;
    ...
    createSphere(radius: number, widthSegments: number = 32, heightSegments: number = 16, options?: GeometryOptions): Geometry {
    ...
    createPlane(width: number, height: number, widthSegments: number = 1, heightSegments: number = 1, options?: GeometryOptions): Geometry {
        const hw = width / 2;
        const hh = height / 2;
     ...
     */
    createGrid(params: { size?: number; divisions?: number; color1?: any; color2?: any } = {}): Geometry {
        ...
        geometry.type = GeometryType.LINES;
     ...
     */
    createPointCloud(data: { points: Float32Array; colors?: Float32Array; normals?: Float32Array; count: number }): Geometry {
        const geometry = this.createGeometry(
            data.points,
            undefined,
            {
     ...
     */
    createTubeFromCurve(curve: any, params: { radius?: number; segments?: number } = {}): Geometry {
     ...
     */
    createWireframe(geometry: Geometry): Geometry {
        if (!geometry.indices) {
        ...
        wireframe.type = GeometryType.LINES;
     ...
     */
    createLine(start: { x: number; y: number; z: number }, end: { x: number; y: number; z: number }): Geometry {
        const positions = new Float32Array([
     ...
     */
    createFromArrays(vertices: Float32Array, indices: Uint32Array, normals?: Float32Array, uvs?: Float32Array): Geometry {
     ...
     */
    createCube(size: number = 1): Geometry {
     ...
     */
    createCylinder(radius: number = 1, height: number = 2, segments: number = 16): Geometry {
        const positions: number[] = [];
        const normals: number[] = [];
        const uvs: number[] = [];
        const indices: number[] = [];
    ...
    createGeometry(vertices: Float32Array, indices: Uint16Array | Uint32Array | undefined, attributes: Record<string, VertexAttribute>): Geometry {
    ...
    setAttribute(geometry: Geometry, name: string, data: Float32Array): void {
    ...
    getAttribute(geometry: Geometry, name: string): Float32Array | undefined {
    ...
    computeNormals(geometry: Geometry): void {
    ...
    computeTangents(geometry: Geometry): void {
        const positions = this.getAttribute(geometry, 'position') || geometry.vertices;
        const normals = this.getAttribute(geometry, 'normal');
        const uvs = this.getAttribute(geometry, 'uv');
    ...
    computeBoundingBox(geometry: Geometry): void {
        const positions = this.getAttribute(geometry, 'position') || geometry.vertices;
    ...
    computeBoundingSphere(geometry: Geometry): void {
        const positions = this.getAttribute(geometry, 'position') || geometry.vertices;
    ...
    getGeometry(id: string): Geometry | undefined {
...
export function createG3DGeometryProcessor(): GeometryProcessor {
