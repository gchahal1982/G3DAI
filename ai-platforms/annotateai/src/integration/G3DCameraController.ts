...
import { Camera } from './G3DNativeRenderer';
...
export class CameraController {
    ...
    constructor(camera: Camera, domElement: HTMLElement, config?: Partial<CameraConfig>) {
        this.camera = camera;
    ...
    private onMouseMove(event: MouseEvent): void {
        ...
        if (this.config.mode === CameraMode.ORBIT) {
        ...
        } else if (this.config.mode === CameraMode.FLY) {
    ...
    private handleKeyboardInput(deltaTime: number): void {
        ...
        if (this.config.mode === CameraMode.FLY ||
            this.config.mode === CameraMode.FIRST_PERSON) {
    ...
    animateTo(position: vec3, target: vec3, duration: number, easing: Easing = Easing.EASE_IN_OUT): Promise<void> {
...
export function createG3DCameraController(
    ...
    return new CameraController(camera, domElement, config);
}