...
// Scene node types
export type NodeType = 'group' | 'mesh' | 'light' | 'camera' | 'annotation' | 'helper';
...
export class SceneManager {
    ...
    private dirtyNodes: Set<SceneNode> = new Set();
    ...
    private updateCallbacks: Map<string, (node: SceneNode, deltaTime: number) => void> = new Map();
    ...
    createNode(name: string, type: NodeType): SceneNode {
    ...
    findNodesByType(type: NodeType): SceneNode[] {
    ...
    private updateNodeTransform(node: SceneNode): void {
    ...
    private updateBoundingBox(node: SceneNode): void {
    ...
    private updateBoundingSphere(node: SceneNode): void {
    ...
    performFrustumCulling(): SceneNode[] {
        const startTime = Date.now();
    ...
    private traverseForCulling(node: SceneNode, visibleNodes: SceneNode[]): boolean {
        if (!node.visible) {
    ...
    private sphereInFrustum(sphere: BoundingSphere): boolean {
        for (const plane of this.frustum.planes) {
    ...
    private boxInFrustum(box: BoundingBox): boolean {
        for (const plane of this.frustum.planes) {
    ...
    private updateRenderObjects(visibleNodes: SceneNode[]): void {
        ...
        for (const node of visibleNodes) {
            if (node.userData.renderObject) {
                const renderObject = node.userData.renderObject as RenderObject;
    ...
    addUpdateCallback(nodeId: string, callback: (node: SceneNode, deltaTime: number) => void): void {
    ...
    createLODGroup(name: string, config: LODConfig): SceneNode {
    ...
    private updateLOD(lodGroup: SceneNode): void {
        const config = lodGroup.userData.lodConfig as LODConfig;
        if (!config) return;
    ...
    private deserializeHierarchy(data: any, parent: SceneNode, nodeMap: Map<string, SceneNode>): void {
     ...
     * Create a camera
     */
    createCamera(type: 'perspective' | 'orthographic', params: any): Camera {
        const camera: Camera = {
     ...
     * Create a light
     */
    createLight(type: 'ambient' | 'directional' | 'point' | 'spot', params: any): Light {
        const light: Light = {
            id: this.generateId(),
            type,
     ...
     * Add object to scene
     */
    add(object: SceneNode | Camera | Light | Mesh | any): void {
     ...
     * Create a mesh
     */
    createMesh(name: string, geometry: any, material: any): Mesh {
        const mesh: Mesh = {
            id: this.generateId(),
            name,
            type: 'mesh',
     ...
     * Get object by name
     */
    getObject(name: string): Mesh | Light | Camera | null {
        // Search in meshes
        for (const mesh of this.meshes.values()) {
            if (mesh.name === name) return mesh;
        }
     ...
     */
    getObjectByName(name: string): Mesh | Light | Camera | SceneNode | null {
        // First check nodes
     ...
     */
    remove(object: SceneNode | Camera | Light | Mesh | any): void {
        if (object.type === 'camera') {
     ...
     * Create a group node
     */
    createGroup(name: string): SceneNode {
    ...
    private testRayMeshIntersection(ray: Ray, mesh: Mesh): RaycastResult | null {
...
export function createG3DSceneManager(renderer: NativeRenderer): G3DSceneManager {
    return new SceneManager(renderer);
}