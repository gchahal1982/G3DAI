...
export default function WorkspaceLayout({ children }: WorkspaceLayoutProps) {
  ...
  const initializeWorkspace = async () => {
    try {
      ...
      if (!user) {
        console.log('🔧 No authentication found, using mock user for workspace');
        user = {
          id: 'dev-user-001',
          ...
          preferences: {
            ...
            workspace: {
              defaultWorkspace: '/workspace/imaging',
              hangingProtocols: [],
              windowLevelPresets: [],
              annotationDefaults: {
                defaultTool: 'arrow',
      ...
      setCurrentUser(user);
      console.log('🔬 Medical workspace initialized for:', `${user.firstName} ${user.lastName}`);
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 text-center">
          ...
          <h2 className="text-xl font-semibold text-slate-900 mb-2">
            Loading MedSight Pro Workspace
          </h2>
          <p className="text-slate-600">
            Initializing medical workspace...
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 text-center max-w-md">
          ...
          <p className="text-slate-600 mb-4">
            Please log in to access the medical workspace.
          </p>
          <button
