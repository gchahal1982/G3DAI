...
export class AIService extends EventEmitter {
  ...
  public async processRequest(request: AIRequest): Promise<AIResponse> {
    ...
    try {
      ...
      const processingTime = performance.now() - startTime;
      this.updateMetrics('request_error', { error: error.message, processingTime });
    ...
    const checkPromises = Array.from(this.models.values()).map(async (model) => {
      try {
        ...
        if (model.health.consecutiveFailures >= 3) {
          model.status = 'error';
          this.emit('model_unhealthy', { modelId: model.id, error });
        }
        
        console.error(`Health check failed for ${model.name}:`, error.message);
      }
    });
  ...
  private getHealthSummary(): object {
    ...
    return {
      ...
      queues: {
        requests: {
          waiting: this.requestQueue.waiting,
          active: this.requestQueue.active,
          completed: this.requestQueue.completed,
          failed: this.requestQueue.failed
        },
        batches: {
...
export { AIService, AIServiceConfig, ModelConfig, AIRequest, AIResponse };
