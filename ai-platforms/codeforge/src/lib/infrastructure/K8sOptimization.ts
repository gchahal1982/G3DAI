 ...
 * K8sOptimization.ts
 * 
 * Advanced Kubernetes optimization system for CodeForge enterprise deployment.
...
class PodOptimizer {
  ...
  private getBaseResources(workloadType: string, size: string): PodOptimizationConfig['resources'] {
    ...
    const base = resourceMap[workloadType][size];
...
class CostOptimizer {
  ...
  optimizeForCost(workload: string, currentCost: number): {
    ...
    if (this.config?.optimization.enableRightSizing) {
...
class PerformanceMonitor {
  ...
  generateAlertingRules(): string {
    ...
    const alertRules = {
      ...
      kind: 'PrometheusRule',
      metadata: {
        name: 'codeforge-alerts',
        namespace: 'monitoring'
      },
      spec: {
        groups: [{
          name: 'codeforge.rules',
