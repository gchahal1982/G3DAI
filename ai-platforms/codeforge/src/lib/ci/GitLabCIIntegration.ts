...
export class GitLabCIIntegration extends EventEmitter {
  ...
  createCITemplates(): Map<string, GitLabTemplate> {
    ...
    // CodeForge CI/CD Template
    templates.set('codeforge-ci', {
      ...
      'build:app': {
        ...
        artifacts: {
          name: 'codeforge-${CI_COMMIT_REF_SLUG}-${CI_COMMIT_SHORT_SHA}',
          paths: ['dist/', 'release/'],
          expire_in: '1 month'
        },
        rules: [
      ...
      'deploy:staging': {
        ...
        script: [
          ...
          'kubectl rollout status deployment/codeforge-app -n staging'
        ],
        environment: {
          name: 'staging',
          url: 'https://staging.codeforge.dev',
          kubernetes: {
            namespace: 'staging'
          }
        },
      ...
      'deploy:production': {
        ...
        script: [
          ...
          'kubectl rollout status deployment/codeforge-app -n production'
        ],
        environment: {
          name: 'production',
