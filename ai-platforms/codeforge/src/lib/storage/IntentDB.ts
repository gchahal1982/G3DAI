...
export class IntentDB extends EventEmitter {
  ...
  public async searchIntents(query: IntentSearchQuery): Promise<IntentSearchResult> {
    ...
    const intents = result.rows.map(row => this.mapRowToIntent(row));
  ...
  private async calculateSearchFacets(query: IntentSearchQuery): Promise<IntentSearchResult['facets']> {
    ...
    return {
      types: Object.fromEntries(results[0].rows.map(r => [r.type, parseInt(r.count)])),
      statuses: Object.fromEntries(results[1].rows.map(r => [r.status, parseInt(r.count)])),
      priorities: Object.fromEntries(results[2].rows.map(r => [r.priority, parseInt(r.count)])),
      complexities: Object.fromEntries(results[3].rows.map(r => [r.complexity, parseInt(r.count)])),
      tags: Object.fromEntries(results[4].rows.map(r => [r.tag, parseInt(r.count)]))
    };
  }
  ...
  public async getCodeLinks(intentId: string): Promise<CodeLink[]> {
    ...
    return result.rows.map(row => this.mapRowToCodeLink(row));
  }
    ...
    for (const intent of this.syncQueue) {
      try {
        ...
        await this.pool.query(`
          ...
          VALUES ($1, $2, $3, $4, $5, $6)
        `, [intent.id, 'push', source, destination, 'failed', error.message]);
      }
    }
