...
export class EnterpriseAuth extends EventEmitter {
  ...
  async authenticate(credentials: AuthCredentials): Promise<AuthResult> {
    ...
    try {
      ...
      auditEvent = auditEvent || AuditEvent.LOGIN_FAILURE;
      await this.logAuditEvent({
        event: auditEvent,
        userId,
        sessionId,
  ...
  private generateQRCode(email: string, secret: string): string {
    ...
    return `otpauth://totp/${encodeURIComponent(email)}?secret=${secret}&issuer=CodeForge`;
  }
  ...
  private getFIDO2Options(): any {
    return {
      challenge: this.generateFIDO2Challenge(),
      rp: { name: 'CodeForge', id: 'codeforge.dev' },
