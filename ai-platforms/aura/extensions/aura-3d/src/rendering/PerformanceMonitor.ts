...
class PerformanceMonitor extends EventEmitter {
  ...
  private setupGPUTimers(ext: any): void {
    ...
    this.timings.gpuQueries.set('render', ext.createQuery());
    this.timings.gpuQueries.set('update', ext.createQuery());
    this.timings.gpuQueries.set('total', ext.createQuery());
  }
  ...
  beginFrame(): void {
    ...
    if (this.gl && this.timings.gpuQueries.get('total')) {
      const ext = this.gl.getExtension('EXT_disjoint_timer_query_webgl2') ||
                  this.gl.getExtension('EXT_disjoint_timer_query');
      if (ext) {
        ext.beginQuery(ext.TIME_ELAPSED_EXT, this.timings.gpuQueries.get('total'));
    }
    }
  }
  ...
  endFrame(): void {
    ...
    this.metrics.rendering = { 
      ...this.counters,
      textures: this.counters.textureBinds // Map textureBinds to textures for interface compatibility
    };
  ...
  private queryGPUTime(ext: any): void {
    ...
    if (ext.getQueryParameter(this.timings.gpuQueries.get('total'), ext.QUERY_RESULT_AVAILABLE)) {
      const gpuTime = ext.getQueryParameter(this.timings.gpuQueries.get('total'), ext.QUERY_RESULT);
