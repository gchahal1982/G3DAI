...
function M._init_lsp_client()
  ...
  -- Register Aura LSP if not already registered
    ...
    configs.aura = {
      default_config = {
        ...
        on_init = function(client)
          print("Aura LSP client initialized")
          state.lsp_client = client
        end,
        on_exit = function(code, signal)
          print("Aura LSP client exited with code:", code)
...
function M._init_completion_source()
  ...
  cmp.register_source('aura', source)
  state.completion_source = source
  
  -- Update cmp configuration to include Aura
    ...
    local items = {
      {
        ...
        documentation = 'Generated by Aura AI',
        insertText = 'function generateFunction() {\n  // AI generated code\n  return true;\n}',
        insertTextFormat = 2, -- Snippet
        data = {
          detail = 'This function was generated by Aura AI based on your context.'
        }
      },
      {
        label = 'optimizeCode',
        ...
        documentation = 'Optimized by Aura AI',
...
function M._init_floating_windows()
  ...
  function M.floating_window.show_ai_chat()
    ...
    local lines = {
      '# Aura AI Assistant',
      '',
      'Welcome to Aura AI! How can I help you today?',
      '',
      '**Available Commands:**',
  ...
  function M.floating_window.show_3d_view()
    local win = M.floating_window.create({
      title = 'Aura 3D Code Visualization',
...
function M._init_commands()
  -- Create main Aura command group
  vim.api.nvim_create_user_command('Aura', function(opts)
    ...
    elseif subcommand == 'reload' then
      ...
      print('Aura: Unknown command. Available: setup, status, models, chat, 3d, reload')
    end
  end, {
    nargs = 1,
    complete = function()
    ...
    desc = 'Aura AI development platform commands'
  })
  
  -- Individual commands
  vim.api.nvim_create_user_command('AuraGenerate', function(opts)
  ...
  vim.api.nvim_create_user_command('AuraExplain', function()
    M.explain_selection()
  end, {
    desc = 'Explain selected code using AI'
  })
  
  vim.api.nvim_create_user_command('AuraRefactor', function()
    M.refactor_selection()
  end, {
    desc = 'Refactor selected code using AI'
  })
  
  vim.api.nvim_create_user_command('AuraTest', function()
    M.generate_tests()
  end, {
    desc = 'Generate tests for current function'
  })
  
  vim.api.nvim_create_user_command('AuraChat', function()
    M.show_ai_chat()
  end, {
    desc = 'Open AI chat interface'
  })
  
  vim.api.nvim_create_user_command('Aura3D', function()
    M.show_3d_view()
  end, {
...
function M._init_model_service()
  -- Initialize connection to Aura model service
  state.current_model = {
    name = 'GPT-4.1',
    provider = 'openai',
    status = 'ready'
