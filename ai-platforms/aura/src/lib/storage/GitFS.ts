...
export class GitFS extends EventEmitter {
  ...
  public async writeFile(filePath: string, content: Buffer | string, options: {
    ...
    const contentBuffer = Buffer.isBuffer(content) ? content : Buffer.from(content as string, (options.encoding || 'utf-8') as BufferEncoding);
  ...
  public async readFile(filePath: string, options: {
    ...
    if (file.compressed) {
      try {
        ...
        throw new Error(`Failed to decompress file: ${(error as Error).message}`);
      }
    }
    ...
    return options.encoding ? content.toString(options.encoding as BufferEncoding) : content;
  }
