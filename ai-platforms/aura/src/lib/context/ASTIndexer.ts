/**
 * ASTIndexer.ts - Incremental AST indexing for Aura dynamic context
 * 
...
export class ASTIndexer extends EventEmitter {
  ...
  searchSymbols(query: string, type?: CodeSymbol['type']): CodeSymbol[] {
    ...
    for (const fileIndex of Array.from(this.fileIndexes.values())) {
      for (const symbol of Array.from(fileIndex.symbols.values())) {
