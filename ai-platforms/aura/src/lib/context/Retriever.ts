...
export class Retriever {
  ...
  private async getCandidateChunks(query: RetrievalQuery): Promise<ContextChunk[]> {
    ...
    if (query.embedding) {
      const vectorResults = await this.vectorDB.search('code-chunks', query.embedding, { limit: 50 });
      for (const result of vectorResults) {
        const chunk = await (this.semanticStore as any).get('chunk', result.id) as ContextChunk | null;
        if (chunk) {
