...
export class XRManager extends EventEmitter {
  ...
  private updateHandTracking(frame: XRFrame): void {
    ...
    for (const inputSource of this.session.session!.inputSources) {
      if (inputSource.hand) {
        const hand = this.createHandData(inputSource.hand as any, frame);
  ...
  private createHandData(xrHand: XRHand, frame: XRFrame): XRHand {
    ...
    // XRHand joint names from WebXR specification
    const jointNames = [
    ...
    for (const jointName of jointNames) {
      const joint = (xrHand as any)[jointName];
      if (joint && this.session?.localSpace && frame.getJointPose) {
    ...
    for (const [controllerId, controller] of this.controllers) {
      if (controller.inputSource) {
        ...
        if (controller.inputSource.targetRaySpace) {
          ...
          controller.pose = targetRayPose || null;
        }
  ...
  private addController(inputSource: XRInputSource): void {
    const controller: XRController = {
      ...
      targetRayMode: inputSource.targetRayMode === 'transient-pointer' ? 'tracked-pointer' : inputSource.targetRayMode as 'tracked-pointer' | 'gaze' | 'screen',
  ...
  private handleInputSourcesChange(event: XRInputSourcesChangeEvent): void {
    ...
    event.added.forEach((inputSource: XRInputSource) => {
      this.addController(inputSource);
    });
  }
  ...
  getSessionInfo(): XRSession | null {
    return this.session as XRSession | null;
  }
