...
class PerformanceMonitor extends EventEmitter {
  ...
  private setupGPUTimers(ext: any): void {
    ...
    this.timings.gpuQueries = queries as any;
  }
  ...
  endFrame(): void {
    ...
    this.metrics.rendering = { ...this.counters, textures: this.counters.textureBinds };
  ...
  private queryGPUTime(ext: any): void {
    ...
    const totalQuery = this.timings.gpuQueries.get('total');
    if (totalQuery && ext.getQueryParameter(totalQuery, ext.QUERY_RESULT_AVAILABLE)) {
      const gpuTime = ext.getQueryParameter(totalQuery, ext.QUERY_RESULT);
