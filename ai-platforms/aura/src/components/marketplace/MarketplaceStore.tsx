...
const MarketplaceStore: React.FC<MarketplaceStoreProps> = ({
  ...
  const getCategoryIcon = (category: PluginCategory) => {
    ...
    return categoryInfo?.icon && typeof categoryInfo.icon === 'object' ? categoryInfo.icon : <ExtensionIcon />;
  };
  ...
  const renderPluginCard = (plugin: MarketplacePlugin) => {
    ...
    return (
      <Box sx={{ width: { xs: '100%', sm: '50%', md: '33.33%', lg: '25%' } }}>
              ...
              <Box display="flex" gap={1} mb={2} flexWrap="wrap">
                {selectedPlugin.categories.map(category => (
                  <Chip 
                    ...
                    label={category.replace('-', ' ')} 
                    size="small" 
                    icon={getCategoryIcon(category) as React.ReactElement}
                  />
                ))}
              </Box>
