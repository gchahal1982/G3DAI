 ...
 * 
 * Comprehensive analytics dashboard for Aura with real-time monitoring.
...
export const AnalyticsDashboard: React.FC = () => {
  ...
  const renderOverviewTab = () => (
    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
      {/* Key Metrics */}
      <Box sx={{ flex: { xs: '1 1 100%', sm: '1 1 50%', md: '1 1 25%' }, minWidth: 0 }}>
        {renderMetricCard(
          'P95 Latency',
              ...
              <Box key={test.id} sx={{ mb: 3, p: 2, border: '1px solid #e0e0e0', borderRadius: 1 }}>
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                  {test.variants.map((variant) => (
                    <Box key={variant.id} sx={{ flex: { xs: '1 1 100%', md: '1 1 50%' }, minWidth: 0 }}>
