...
interface FeedbackButtonsProps {
  ...
  context?: Partial<FeedbackContext> | undefined;
  codeContext?: CodeContext | undefined;
  onFeedbackSubmitted?: ((feedback: Feedback) => void) | undefined;
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={1} flexWrap="wrap" gap={1}>
        {options.map((option) => (
          // @ts-ignore - MUI exactOptionalPropertyTypes conflict
          <Chip
            key={option.id}
            label={option.label}
            {...(option.icon && React.isValidElement(option.icon) ? { icon: option.icon } : {})}
            clickable
